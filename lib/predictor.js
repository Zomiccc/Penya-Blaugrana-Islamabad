...
function getCurrentRound(matches, now = new Date(), competitionCode) {
  ...
  // A round counts as still in play until its last kickoff is three hours
  // old, the same allowance hasFinalScore() makes for extra time.
  const FULL_MATCH_MS = 3 * 60 * 60 * 1000;
  const inPlay = roundBlocks(inCompetition, competitionCode)
    .filter((block) => hasKickedOff(block[0], now))
    .filter((block) => new Date(block[block.length - 1].utcDate).getTime() + FULL_MATCH_MS > now.getTime())
