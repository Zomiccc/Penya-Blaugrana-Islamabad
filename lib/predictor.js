...
function getCurrentRound(matches, now = new Date(), competitionCode) {
  ...
  // The round the TABLE describes is the one that has actually started —
  // deliberately not the round that's currently open for predictions.
  //
  // Taking it from the prediction window meant the weekly table jumped to
  // the new match week the instant the previous one's last match kicked off,
  // blanking the completed week's results and showing zeros for a week
  // nobody had played yet. Nothing is revealed before its match begins.
  //
  // Keyed on the most recently kicked-off match rather than "does this round
  // have any started match", because leagues reschedule: a match week 6
  // fixture can be played weeks early, and that must not drag the table
  // forward while match week 5 is still being played.
  ...
  // Nothing played yet (start of a season) — fall back to the round that's
  // coming up, so the table at least names the right week while empty.
