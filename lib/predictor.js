/* ==========================================================================
   ...
   1. WINDOW      Members predict one competition's round (matchday) at a
                  ...
                  in one set.
   2. DEADLINE    A match locks at its own kickoff. The ROUND stays current
                  until its last match has finished; only then does the next
                  round open for predictions.
...
/**
 * Has this match been played out? Used to decide when a match week is done
 * with, so the next one can open. Trusts the feed's FINISHED status, with a
 * three-hour-past-kickoff fallback in case a sync is missed — that covers
 * extra time and penalties, so a week can never stick open forever.
 */
function isOver(match, now = new Date()) {
  if (!match) return true;
  if (match.status === 'FINISHED') return true;
  return new Date(match.utcDate).getTime() + 3 * 60 * 60 * 1000 <= now.getTime();
}

/**
 * The round a competition is currently on: the earliest one that has not yet
 * been played out in full.
 *
 * A match week stays current until its LAST match has finished. Not until
 * that match kicks off, and not until the week's first match kicks off —
 * while any game of the week is still to be played, that is the week the
 * club is on, and the next week does not exist yet.
 *
 * Both the prediction set and the weekly table are drawn from this, so the
 * page can never offer week 7 while week 6 still has matches left.
 */
function currentRoundBlock(matches, now = new Date(), competitionCode) {
  ...
  const unfinished = roundBlocks(matches, competitionCode)
    .filter((block) => block.some((m) => !isOver(m, now)))
    .sort((a, b) => medianTime(a) - medianTime(b));

  return unfinished.length ? unfinished[0] : [];
}

/**
 * The prediction set for one competition: the fixtures of the current round
 * that have not kicked off yet.
 *
 * Each match locks at its own kickoff and drops out of the set; the round
 * itself stays current until its last match has finished, and only then does
 * the next round open. So members can still enter the rest of a week while
 * it is being played, and a new week never appears while the old one has
 * games outstanding.
 *
 ...
 *   League). Omit to auto-pick whichever competition is on the earlier round.
 */
function getPredictionWindow(matches, now = new Date(), competitionCode) {
  return currentRoundBlock(matches, now, competitionCode)
 ...
 * The next thing to close: kickoff of the soonest fixture still open. Each
 * match locks at its own kickoff, so this is what the countdown on the page
 * is counting down to.
 */
...
function getCurrentRound(matches, now = new Date(), competitionCode) {
  ...
  // The table shows the week the club is on — the same week members are
  // predicting — and turns over only once that week's last match has
  // finished. While it is the coming week it lists everyone who has entered
  // at 0 points / 0 exact: WHO has entered, never WHAT they picked. The
  // scorelines stay sealed until each match kicks off, enforced per match in
  // the results endpoint rather than here. Earlier weeks stay readable
  // through the week picker.
  const round = currentRoundBlock(inCompetition, now, competitionCode);
  if (round.length) {
    return { matchday: round[0].matchday ?? null, seasonId: round[0].season?.id ?? null };
  }
