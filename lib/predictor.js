...
function getPredictionWindow(matches, now = new Date(), competitionCode) {
  ...
  // fixture. Leagues reschedule individual games weeks out of order — La
  // Liga moved a matchday-6 game to 3 September, ahead of all of matchday 4
  // — and keying on the earliest fixture let that one game drag the whole
  // window forward, so members were offered matchday 6 while matchdays 4
  // and 5 were still to be played.
  ...
  const medianTime = (group) =>
    new Date(group[Math.floor(group.length / 2)].utcDate).getTime();

  const [round] = [...groups.values()].sort((a, b) => medianTime(a) - medianTime(b));
  // `upcoming` was sorted soonest-first and grouping preserved that order.
  return round;
}
