...
function defaultDb() {
  return {
    ...
    broadcasts: [],
    pushSubscriptions: [],
    // Same, but per member: { memberId, endpoint, subscription, createdAt }
    memberPushSubscriptions: [],
    // { [memberId]: { firstName, lastName, restoredAt } } — lets the admin
    // manually re-attach a name to predictions/chat made by a member who was
    // later deleted, WITHOUT resurrecting their account (no login, no
    // membership record). Kept separate from `members` on purpose so it can
    // never interfere with member management, stats, or login.
    deletedMemberNames: {},
    // Audit trail for predictions the admin removed (test accounts,
    // duplicates). Stored scores are never edited — only removed — and a
    // copy is kept here so a removal can be reviewed or undone by hand.
    deletedPredictions: [],
  };
}
...
function readJsonDb() {
  ...
  if (db.chatMessages === undefined) db.chatMessages = [];
  if (db.chatConversations === undefined) db.chatConversations = [];
  if (db.broadcasts === undefined) db.broadcasts = [];
  if (db.pushSubscriptions === undefined) db.pushSubscriptions = [];
  if (db.memberPushSubscriptions === undefined) db.memberPushSubscriptions = [];
  if (db.deletedMemberNames === undefined) db.deletedMemberNames = {};
  if (db.deletedPredictions === undefined) db.deletedPredictions = [];
  return db;
}
