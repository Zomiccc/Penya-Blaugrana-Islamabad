...
/**
 * Send a member the 6-digit code they need to set (or reset) their Predictor
 * League password. Short-lived and single-use — see server.js.
 * @param {object} member  The member requesting the code.
 * @param {string} code    The 6-digit code.
 * @param {number} minutes How long the code stays valid.
 */
async function sendMemberAuthCode(member, code, minutes) {
  if (!enabled) {
    console.warn('[mailer] sendMemberAuthCode called but SendGrid is not enabled');
    return;
  }
  const msg = {
    to: member.email,
    from: { email: FROM_EMAIL, name: FROM_NAME },
    subject: `Your Predictor League code: ${code}`,
    text: [
      `Hola ${member.firstName},`,
      '',
      'Use this code to set your password for the Penya Blaugrana Islamabad Predictor League:',
      '',
      `    ${code}`,
      '',
  ...
  await sgMail.send(msg);
  console.log('[mailer] Predictor League code sent to:', member.email);
}
