...
const MATCHDAY_INTERVAL_MS = 5 * 60 * 1000;     // 5 minutes
...
function desiredIntervalMs(cache) {
  ...
  // If the next match is today (same calendar date in UTC), poll every 5 min.
  const sameUtcDay =
...
async function runSync(reason) {
  ...
  try {
    ...
    if (interval !== lastScheduledIntervalMs) {
      console.log(`[fixtures] refresh interval changed to ${interval === MATCHDAY_INTERVAL_MS ? '5 min (match day)' : '12 hours (normal)'}`);
